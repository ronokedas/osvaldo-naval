  ...
  return <div className="space-y-6">
    ...
    <div className="divide-y overflow-hidden rounded-2xl border bg-white shadow-sm">
      ...
      {filtered.map((protocol) => {
        ...
        const deliveryStillOpen = ['pendente', 'em_entrega'].includes(protocol.entregaStatus || '');
        return <div key={protocol.id} className="space-y-4 p-5">
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row">
            ...
            <div className="flex flex-wrap items-start gap-2">
              ...
              {protocol.status === 'aprovado' && pendingFinalDocuments.length === 0 && !deliveryStillOpen && currentUser.role === 'admin' && <button onClick={() => setSupplementalDocumentProtocol(protocol)} className="rounded-lg bg-orange-600 px-3 py-2 text-xs font-bold text-white">Adicionar documento suplementar e reabrir entrega</button>}
