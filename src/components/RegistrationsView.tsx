...
export function RegistrationsView({ onChanged }: { onChanged?: () => void }) {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Help Banner: Vincular Embarcações a Clientes */}
      <div className="bg-gradient-to-r from-blue-900 to-slate-900 text-white rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border border-blue-800/50">
        <div className="flex items-start gap-3">
          <div className="p-2.5 bg-blue-600/30 rounded-xl border border-blue-400/30 shrink-0">
            <Ship className="w-5 h-5 text-blue-300" />
          </div>
          <div>
            <h3 className="font-bold text-sm text-white flex items-center gap-2">
              Dica: Como vincular Embarcações a um Cliente?
            </h3>
            <p className="text-xs text-slate-300 mt-0.5">
              Cada cliente pode possuir várias embarcações vinculadas. Para vincular uma nova embarcação, vá na aba{' '}
              <strong className="text-cyan-300">Embarcações</strong> ➔ <strong className="text-cyan-300">+ Nova Embarcação</strong> e selecione o cliente no campo Proprietário.
            </p>
          </div>
        </div>
      </div>

      {/* 3 Main Registration Columns */}
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4 flex flex-col justify-between">
          <div>
            ...
            <form onSubmit={handleSaveClient} className="mt-4 grid gap-3 sm:grid-cols-2 text-xs">
              <input
                required
                ...
                return (
                  <div
                    ...
                    className={`rounded-xl p-3 text-xs border transition flex items-start justify-between gap-2 ${
                      editingClientId === c.id ? 'bg-blue-50 border-blue-300 ring-2 ring-blue-200' : 'bg-slate-50 hover:bg-slate-100/80 border-slate-200/80'
                    }`}
                  >
        ...
        {/* COLUMN 2: CERTIFICADORAS */}
        <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b pb-3">
              <h2 className="font-bold text-slate-900 text-base flex items-center gap-2">
                <Building2 className="w-5 h-5 text-indigo-600" />
                {editingCertifierId ? 'Editar Certificadora' : 'Nova Certificadora'}
              </h2>
