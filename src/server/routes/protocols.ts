...
router.post("/", requirePermission([PERMISSIONS.REGISTRAR_ENVIO_RESPOSTA_EXTERNA]), async (req, res) => {
  try {
    const data = req.body;
...
router.put("/:id", requirePermission([PERMISSIONS.REGISTRAR_ENVIO_RESPOSTA_EXTERNA]), async (req, res) => {
  try {
    const { id } = req.params;
    const data = req.body;
